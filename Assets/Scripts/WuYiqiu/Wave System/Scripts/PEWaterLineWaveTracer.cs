...
public class PEWaterLineWaveTracer : LineWaveTracer 
{
	public string Desc;
	...
	public override void CustomUpdate ()
	{
		...
		if (Length > 5 || Width > 5) {
			// Safecheck for multiplay's deadloop bug(unknown reason)
			Debug.LogError("[PEWaterLineWaveTracer]Too large number:"+Length+","+Width);
			Length = Width = 1;
		}
		...
		bool in_water = false;
		for (float x = min_x; x < max_x + 0.51f; x += 1)
		{
			for (float z = min_z; z < max_z + 0.51f; z += 1)
			{
