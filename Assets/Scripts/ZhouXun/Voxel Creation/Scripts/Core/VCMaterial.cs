...
// Iso's materials was saved as VCMaterial[], the index of each material will apply to voxel types.
...
public class VCMaterial
{
	...
	public void LoadCustomizeTexture(string path_d, string path_n)
	{
		...
		catch (Exception)
		{
			...
			has_diffuse = false;
			m_DiffuseTex = Texture2D.Instantiate(Resources.Load(VCMaterial.s_BlankDiffuseRes)) as Texture2D;
		}
		m_DiffuseData = m_DiffuseTex.EncodeToPNG();
		...
		catch (Exception)
		{
			...
			if ( has_diffuse )
				m_BumpTex = Texture2D.Instantiate(Resources.Load(VCMaterial.s_BlankBumpRes_)) as Texture2D;	// Have diffuse, no bump map
			else
				m_BumpTex = Texture2D.Instantiate(Resources.Load(VCMaterial.s_BlankBumpRes)) as Texture2D;		// Don't have diffuse or bump map
		}
		m_BumpData = m_BumpTex.EncodeToPNG();
