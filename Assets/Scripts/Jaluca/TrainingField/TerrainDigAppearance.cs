...
namespace TrainingScene
{
	public class TerrainDigAppearance : MonoBehaviour
	{
		...
		void LateUpdate()
		{
			for(int i = 0, j = 0; i < VFVoxelTerrain.self.transform.childCount && j < 2; i++)
			{
				GameObject t = VFVoxelTerrain.self.transform.GetChild(i).gameObject;
				if(t.activeSelf && t.name == "Chnk_0_0_0_0")
				{
					j++;
					if(t.GetComponent<VFVoxelChunkGo>().OriginalChunkGo == null)
					{
