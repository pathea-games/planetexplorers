...
Shader "Voxelform/Triplanar 2/Noise Blend/Diffuse Normal Specular" {
    ...
    Properties {
		...
		_NoiseVal1 ("Noise Value 1", Float) = 524287.0
    ...
    SubShader {
		...
		void surf (Input IN, inout CustomSurfaceOutput o)
		{
			...
			// Sample bump maps too, and generate bump vectors. 
			float spec1 = lerp(tex2D(_WallSpecular, coord1).x, tex2D(_FloorSpecular, coord1).x, noise); 
			float spec2 = lerp(tex2D(_WallSpecular, coord2).x, tex2D(_FloorSpecular, coord2).x, noise); 
			float spec3 = lerp(tex2D(_WallSpecular, coord3).x, tex2D(_FloorSpecular, coord3).x, noise);
