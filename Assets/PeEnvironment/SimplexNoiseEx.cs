...
public class SimplexNoiseEx
{
    ...
    private static double F2 = 0.5 * (Mathf.Sqrt(3.0f) - 1.0);
    private static double G2 = (3.0 - Mathf.Sqrt(3.0f)) / 6.0;
	...
	public SimplexNoiseEx()			{	InitMember();		}
	public SimplexNoiseEx(long seed)	{	InitMember(seed);	}
	...
	{
        double n0, n1; // Noise contributions from the three corners
		...
		double t0 = 1 - x0*x0;
		t0 *= t0;
		n0 = t0 * t0 * grad3[perm[i0&0xff]&7].x * x0;
	
		double t1 = 1 - x1*x1;
		t1 *= t1;
		n1 = t1 * t1 * grad3[perm[i1&0xff]&7].x * x1;
		...
		return (n0 + n1 - 0) * 3.1604938271604;	// 2.45488110001;
	}
